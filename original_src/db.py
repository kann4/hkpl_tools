...
def getCopies(library_name,
              libraries_to_remove):  #get copies availabe in certain libraary
    try:
        sqliteConnection = get_db_connection()
        cursor = sqliteConnection.cursor()
        ...
        cursor.execute(sqlite_select_query,
                       (library_name, *libraries_to_remove
                        ))  # (library_name,) + tuple(libraries_to_remove)
...
def insertCopies(
    book_ID, copies, cursor
):  #contain query code only, use for insertIntoTables() and updateCopies()
    for copy in copies:
...
def delBook(book_IDs):
    try:
        ...
        for book_ID in book_IDs:
            ...
            cursor.execute("DELETE from BookCopy WHERE bookID = ?",
                           (book_ID, ))
        sqliteConnection.commit()
        cursor.close()
        return book_IDs
