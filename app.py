...
from book_service import (
    get_book_table,
    del_book,
    add_book_by_bib_or_url,
    update_all_copies,
    update_current_copies,
    get_last_update,
    get_list_of_libraries,
    get_copies,
)
...
def sort_by(collection, *attributes):
    ...
    return sorted(
        collection,
        key=lambda x: tuple(x[attr] if attr in x else 9999 for attr in attributes),
    )
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            #     return render_template('index.html', libraries_to_remove=libraries_to_remove, libraries=libraries, selection=library)
                ...
                'btnUpdateAll' in request.form or 'btnUpdateCurrent' in request.form
            ):  # update
                if 'btnUpdateAll' in request.form:
                    ...
                    library = request.form['library']
                    libraries_to_remove = request.form.getlist('libraryToRemove')
                    books_failed_to_update = update_current_copies(
                        library, libraries_to_remove
                    )
                ...
                return render_template(
                    'index.html', update_msg=update_msg, libraries=libraries
                )
            else:
                print(request.form)
                return render_template(
...
def saved_books():
    ...
    if request.method == 'POST':
        ...
        table = get_book_table()
        return render_template(
            'books.html', column_names=table[0], books=table[1], msg=msg
        )
